...
layui.define(['jquery', 'form', 'layer', 'element'], function(exports) {
	...
	$(function() {
		...
		function setMenu(){
			...
			$.getJSON(url,function(data){
				dataStr = data.menu;
				$('#side-nav #nav').html(createMenu(dataStr));
				// element.init();
			});
		}
		setMenu();
		...
		$(window).resize(function() {
			...
			if($('.left-nav').css('left') == '0px') {
				$('.footer').css("width", "calc(100% - 221px)");
			} else {
	...
	// $('.container dd').on('mousemove', function(){
	// 	$(this).addClass('layui-this');
	// });

	// $('.container dd').on('mouseout', function(){
	// 	$(this).removeClass('layui-this');
	// });

	$('.container dd').on('click', function(){
		$(this).removeClass('layui-this');
	});
	...
	$('.fast-add dl a').on('click', function(){
		...
		if(type == "closeall") {
			...
			$(".page-content .layui-tab-content .layui-show").find("iframe")[0].contentWindow.location.reload();
